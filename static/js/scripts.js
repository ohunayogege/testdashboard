...
$(".form-login").on("submit", function(e){
  ...
  $.ajax({
    ...
    error: function(res){
      Swal.fire("Network Error", "An error occurred. Please try again later.", "error");
    }
  });
});

$(".inter-bank").on("submit", function(e){
  e.preventDefault();
  const fd = $(this);
  ...
  $.ajax({
    ...
    dataType: "json",
    beforeSend: function(){
      $(".btn-inter").attr("disabled", "disabled");
      $(".btn-inter").text("Checking...");
    },
    complete: function(){
      $(".btn-inter").text("Continue");
      $(".btn-inter").removeAttr("disabled", "disabled");
    },
    success: function (res) {
      if (res.status == true){
        let modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('pinConfirm')) // Returns a Bootstrap modal instance
        // Show or hide:
        $(".account-name").text(res.data.account_name);
        $(".account-number").text(res.data.account_number);
        $(".account-amount").text(res.data.amount);
        modal.show();
      } else {
        Swal.fire("Transfer Error", res.message, "error");
      }
    },
    error: function(res){
      Swal.fire("Network Error", "An error occurred. Please try again later.", "error");
...
$(".confirm-inter").on("submit", function(e){
  e.preventDefault();
  const fd = $(this);
  ...
  $.ajax({
    ...
    dataType: "json",
    beforeSend: function(){
      $(".btn-inter-go").attr("disabled", "disabled");
      $(".btn-inter-go").text("Proceeding...");
    },
    complete: function(){
      $(".btn-inter-go").text("Proceed");
      $(".btn-inter-go").removeAttr("disabled", "disabled");
    },
    success: function (res) {
      if (res.status == true){
        let modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('pinConfirm')) // Returns a Bootstrap modal instance
        // Show or hide:
        modal.hide();
        Swal.fire("Transfer Successful", res.message, "success");
        setTimeout(() => {
          window.location.reload();
        }, 3000);
      } else {
